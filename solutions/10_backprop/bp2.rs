// bp2.rs — SOLUTION · Chapter 10: Backprop
// Gradients flow THROUGH a hidden layer back to the first weight matrix — but
...
fn grads() -> (Vec<f32>, Vec<f32>) {
    ...
    let g = loss.backward();
    let gw1 = w1.grad(&g).unwrap(); // gradient reached the FIRST layer
