...
//
// The bug: `run_inference` is hard-wired to `NdArray`, so it is NOT generic. The
// call site uses a turbofish (`::<NdArray>`) to choose the backend — which only
...
// ⬇️ NOT generic: hard-wired to NdArray, so the ::<NdArray> call site won't compile
fn run_inference() -> Tensor<NdArray, 2> {
    let device = Default::default();
    let model = Classifier::<NdArray>::new(&device);
    let x = Tensor::<NdArray, 2>::from_floats([[1.0, 2.0, 3.0]], &device);
    model.forward(x)
}
