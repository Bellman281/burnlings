// net3.rs — Burnlings · Chapter 11: A Network From Scratch
//
// The manual SGD update is `param <- param - lr * grad`: gradient DESCENT moves
// AGAINST the gradient. The loop below ADDS the gradient (ascent), so the loss
// climbs and the net never learns XOR.
//
// TODO: Subtract the gradient in every update (change `+` to `-`).
...
fn final_loss() -> f32 {
    ...
    for _ in 0..20000 {
        ...
        // ⬇️ wrong sign: descent SUBTRACTS the gradient, this adds it
