...
// tracking. Notice the return type is the BASE backend `Flex`, not `Autodiff`.
//
// The body never drops the wrapper, so it returns an `Autodiff<Flex>` tensor
// where an `Flex` tensor is expected — and it won't compile.
//
...
fn infer() -> Tensor<Flex, 1> {
    ...
    // ⬇️ still an Autodiff tensor — the return type wants a plain Flex tensor
    x.add_scalar(5.0)
}
